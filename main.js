...
const learningSnippet = (learning = {}) => {
  const lesson = Array.isArray(learning.lessons) ? clean(learning.lessons[0]) : '';
  const decision = Array.isArray(learning.decisions) ? clean(learning.decisions[0]) : '';
  return lesson || decision;
};

const appendLearningSnippet = (container, learning = {}) => {
  const snippet = learningSnippet(learning);
  if (!snippet) return;

  const note = makeEl('p', 'learning-note', `Learned: ${snippet}`);
...
const renderFeaturedProjects = (projects = []) => {
  ...
  projects.slice(0, 3).forEach((project) => {
    ...
    appendLearningSnippet(body, project.learning);
...
const renderOtherProjects = (projects = []) => {
  ...
  projects.forEach((project) => {
    ...
    appendLearningSnippet(card, project.learning);
